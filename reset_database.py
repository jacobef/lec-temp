...
5. Creates all the groups that correspond to account types (everything in LECUser.ALL_ACCOUNT_TYPES)
...
from LEC.settings import BASE_DIR
...
def create_test_user(*args, **kwargs):
    ...
    user = LECUser(*args, **kwargs)

    password = "blueprinttesting"
    user.set_password(password)

    user.save()
    print(f"Created user \"{user}\", with args: {args} and kwargs: {kwargs} and password: \"{password}\"")
    return user
...
def create_account_type_groups():
    ...
    for account_type in LECUser.ALL_ACCOUNT_TYPES:
        ...
        if account_type == LECUser.AccountTypes.SITE_ADMIN:
...
def make_interactive(function: Callable[[], Any], what_happens: str, manual_instructions, required: bool):
    proceed_automatically = y_n_q_input(f"""{what_happens} 
Would you like this to be done automatically?""")
...
def y_n_q_input(message=""):
    result = input(f"""{message} 
(y/n) (q to quit) """)
    while result not in ['y', 'n', 'q']:
        result = input("Invalid input. Enter y, n, or q.")
    if result == 'y':
        return True
    elif result == 'n':
        return False
...
""")
everything_automatic = y_n_q_input(
...
if everything_automatic:
    remove_migrations_files()
    remove_database()
    create_migrations_files()
    create_database()
    ...
    make_interactive(function=create_migrations_files,
    ...
    make_interactive(function=create_account_type_groups,
                     what_happens=
                     "For each possible account type (e.g. site admin, guardian), a group will be created that corresponds to that account type.",
                     manual_instructions=
                     f"""You can do this programmatically, or using Django's auto-generated admin page.

PROGRAMMATICALLY:
    Run Django's Python shell. To do that:
        {generate_managedotpy_manual_instructions("shell")}

    Then, run the following code in that Python shell (or anything with equivalent functionality if you want to change it for whatever reason):
...
USING THE ADMIN PAGE:
    ...
    In the left bar, where it says "Groups", click "Add". For each account type in LECUser.ALL_ACCOUNT_TYPES
    (currently: {LECUser.ALL_ACCOUNT_TYPES}), add a group with that name. (After you add the first group,
...
They will all have the name "Jane Doe", the email "user@example.com", and the password "blueprinttesting".""",
                     manual_instructions=
                     f"""Run the server and go to /accounts/register. For each account type (currently: {LECUser.ALL_ACCOUNT_TYPES}),
add a user called test_[account type], e.g. test_{LECUser.ALL_ACCOUNT_TYPES[0]}
